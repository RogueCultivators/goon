...
func InitProject(projectName, moduleName string, minimal bool, example bool) error {
	...
	if err := createProjectDirectories(projectName, minimal); err != nil {
		return err
	}
	...
	if err := renderProjectFiles(projectName, renderer, data, minimal); err != nil {
		return err
	}

	if example {
		if err := generateExampleModule(projectName, moduleName, renderer); err != nil {
			return err
		}
	}
...
func createProjectDirectories(projectName string, minimal bool) error {
	coreDirs := []string{
	...
	fullDirs := []string{
		"internal/sqlc/queries", "internal/sqlc/schema", "pkg/validator", "pkg/database",
		"pkg/jwt", "pkg/utils", "pkg/cache", "pkg/email", "pkg/upload", "pkg/pagination", "pkg/testutil",
	}
...
func renderProjectFiles(projectName string, renderer *template.Renderer, data template.ProjectData, minimal bool) error {
	files := getCoreFiles()
	if !minimal {
		for k, v := range getFullFiles() {
			files[k] = v
		}
	}
...
func getCoreFiles() map[string]string {
	return map[string]string{
		...
		".env.example": ".env.example.tmpl", "docker-compose.dev.yml": "docker-compose.dev.yml.tmpl",
		"scripts/setup.sh": "setup.sh.tmpl", "scripts/seed.sh": "seed.sh.tmpl",
		"docs/api.md": "api.md.tmpl",
	}
}

func getFullFiles() map[string]string {
	return map[string]string{
		"internal/middleware/requestid.go": "requestid.go.tmpl", "internal/middleware/ratelimit.go": "ratelimit.go.tmpl",
		"internal/middleware/auth.go": "auth.go.tmpl", "internal/middleware/permission.go": "permission.go.tmpl",
		"internal/middleware/gzip.go": "gzip.go.tmpl", "pkg/validator/validator.go": "validator.go.tmpl",
		"pkg/database/database.go": "database.go.tmpl", "pkg/jwt/jwt.go": "jwt.go.tmpl",
		...
		"sqlc.yaml": "sqlc.yaml.tmpl", "Dockerfile": "Dockerfile.tmpl",
		"docker-compose.yaml": "docker-compose.yaml.tmpl",
	}
}

func generateExampleModule(projectName, moduleName string, renderer *template.Renderer) error {
